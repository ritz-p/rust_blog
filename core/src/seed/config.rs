...
pub trait PathConfigTrait {
    fn new(fixed_content_path: Option<String>, article_path: Option<String>) -> Self;
...
mod tests {
    ...
    fn new_uses_defaults_when_none_is_passed() {
        let config = PathConfig::new(None, None);
    ...
    fn new_uses_given_values_when_some_is_passed() {
        let config = PathConfig::new(Some("fixed".to_string()), Some("articles".to_string()));
    ...
    fn update_replaces_all_fields() {
        let mut current = PathConfig::new(None, None);
